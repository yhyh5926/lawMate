  ...
  return (
    <div style={{ maxWidth: "400px", margin: "80px auto", padding: "30px", background: "#fff", borderRadius: "12px", boxShadow: "0 10px 30px rgba(0,0,0,0.1)" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        <button onClick={() => handleSocialClick("google")} style={{ ...socialBtnStyle, backgroundColor: "#fff", border: "1px solid #ddd", color: "#333" }}>Google 계정으로 로그인</button>
        <button onClick={() => handleSocialClick("kakao")} style={{ ...socialBtnStyle, backgroundColor: "#FEE500", color: "#3c1e1e" }}>카카오 로그인</button>
        <button onClick={() => handleSocialClick("naver")} style={{ ...socialBtnStyle, backgroundColor: "#03C75A", color: "#fff" }}>네이버 로그인</button>
      </div>
    </div>
  );
};
