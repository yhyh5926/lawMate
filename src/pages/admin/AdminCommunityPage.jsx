...
const AdminCommunityPage = () => {
  ...
  const fetchBoardData = async () => {
    ...
    try {
      ...
      // 💡 3. 의견 조사(모의판결) 데이터 정규화 추가
      const normalizedPolls = (polls || []).map(poll => ({
        id: poll.pollId,
        type: 'POLL',
        typeLabel: '의견 조사',
        badgeColor: '#fef3c7', textColor: '#b45309', // 노란빛 배지
        title: poll.title,
        author: poll.name || `회원 ${poll.memberId}`,
        status: poll.status,
        createdAt: poll.createdAt,
        detailUrl: `/poll/detail/${poll.pollId}` // 프론트엔드 라우팅에 맞게 조정해주세요
      }));
            ...
            <tbody>
              {filteredData.length > 0 ? filteredData.map((item, idx) => (
                <tr key={`${item.type}-${item.id}-${idx}`}>
                  ...
                  <td style={{fontSize: "12px", color: "#94a3b8"}}>{item.createdAt?.split('T')[0] || item.createdAt?.split(' ')[0]}</td>
                  <td>
