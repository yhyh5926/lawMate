...
export default function LawyerJoinTermsPage() {
  ...
  const handleSingleCheck = (e) => {
    ...
    newTerms.all =
      newTerms.service && newTerms.privacy && newTerms.lawyerPolicy;
    setTerms(newTerms);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-6 bg-white shadow-md rounded-md">
      ...
      <div className="space-y-4 mb-8">
        ...
        <div>
          ...
          <div className="h-24 overflow-y-scroll text-sm text-gray-600 border p-3 rounded bg-gray-50">
            제1조 (자격 증명) 전문회원은 가입 시 입력한 변호사 자격번호가 허위일
            경우 즉시 계정이 정지되며...
            <br />
            제2조 (수수료 및 정산) 상담 결제 시 발생하는 수수료율은 플랫폼
            정책에 따르며...
          </div>
        </div>
      </div>
      ...
      <button
        onClick={handleNext}
        className={`w-full py-3 rounded-md font-bold text-white transition-colors ${terms.service && terms.privacy && terms.lawyerPolicy ? "bg-indigo-700 hover:bg-indigo-800" : "bg-gray-400"}`}
      >
        전문회원 정보 입력 단계로
      </button>
    </div>
