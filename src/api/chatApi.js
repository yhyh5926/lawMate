...
export const getOrCreateChatRoom = (targetMemberNo) =>
  axiosInstance.post('/api/chat/rooms', { targetMemberNo });

// 채팅방 메시지 목록 조회 (페이징)
export const getChatMessages = (roomNo, page = 0, size = 30) =>
  axiosInstance.get(`/api/chat/rooms/${roomNo}/messages`, { params: { page, size } });

// 메시지 읽음 처리
export const markMessagesRead = (roomNo) =>
  axiosInstance.put(`/api/chat/rooms/${roomNo}/read`);

// 채팅방 나가기
export const leaveChatRoom = (roomNo) =>
  axiosInstance.delete(`/api/chat/rooms/${roomNo}`);
