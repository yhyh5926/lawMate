...
export const createPaymentOrder = (consultNo) =>
  axiosInstance.post('/payment/order', { consultNo });

// 결제 완료 검증 (PG사 콜백 후 서버 검증)
export const verifyPayment = (impUid, merchantUid) =>
  axiosInstance.post('/payment/verify', { impUid, merchantUid });

// 결제 내역 목록
export const getPaymentList = () =>
  axiosInstance.get('/payment/my');

// 결제 상세
export const getPaymentDetail = (paymentNo) =>
  axiosInstance.get(`/payment/${paymentNo}`);
...
export const getSettlementList = (year, month) =>
  axiosInstance.get('/payment/settlement', { params: { year, month } });

// 월별 정산 차트 데이터 (Python API)
export const getSettlementChart = () =>
  axiosInstance.get('/payment/settlement/chart');