  ...
  const fetchMyPosts = async (type) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (type === "question") {
          setPosts([
            { id: 101, title: `${user.name}님이 작성하신 전세금 반환 질문입니다.`, date: "2026-03-05", commentCount: 2, status: "답변완료" },
            { id: 102, title: "노동청 신고 절차 질문드립니다.", date: "2026-02-28", commentCount: 0, status: "대기중" },
          ]);
        } else if (type === "community") {
          setPosts([
            { id: 201, title: `${user.name}님이 쓰신 자유게시판 글!`, date: "2026-03-04", commentCount: 5, category: "자유수다" },
          ]);
        } else if (type === "mockTrial") {
          // 💡 새로 추가된 모의 판결 게시판 가짜 데이터
          setPosts([
            { id: 301, title: "층간소음 문제, 과연 누구의 잘못일까요? (모의 판결)", date: "2026-03-01", commentCount: 15, category: "투표진행중" },
            { id: 302, title: "주차장 문콕 뺑소니건 판결 결과입니다.", date: "2026-02-20", commentCount: 8, category: "판결완료" },
          ]);
        }
        setLoading(false);
      }, 500);
              ...
              <li key={post.id} className="mypost-item" onClick={() => handlePostClick(post.id)}>
                <div className="mypost-item-main">
                  ...
                  {subTab === "question" ? (
                    ...
                    <span className={`mypost-badge ${post.category === "판결완료" ? "badge-success" : "badge-normal"}`}>
                      {post.category}
                    </span>
                  ) : (
